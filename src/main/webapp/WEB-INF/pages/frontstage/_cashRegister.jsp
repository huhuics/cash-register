			...
			<table class="table table-bordered table-hover" style="font-family: 'Microsoft YaHei';">
				...
				<tbody>
					<tr v-for="item in goods_list" style="height: 30px; line-height: 30px;">
						...
						<td><input class="form-control" type="text" v-model="item.goodsDiscount" @keyup.enter="editItemDiscountById(item.goodsId,item.goodsDiscount)"></td>
						<td><input class="form-control" type="text" v-model="item.totalActualAmount" @keyup.enter="editItemPriceById(item.goodsId,item.totalActualAmount)"></td>
						<td><input class="form-control" type="text" v-model="item.goodsCount" @keyup.enter="editItemCountById(item.goodsId,item.goodsCount)"></td>
				...
				<tbody>
					<tr>
						...
						<td class="col-xs-4" style="vertical-align: middle;"><div>
								<button type="button" class="btn btn-danger btn-lg btn-block" @click="checkout">收款&nbsp;&nbsp;￥{{summary_price}}</button>
							</div></td>
					</tr>
				</tbody>
			</table>
